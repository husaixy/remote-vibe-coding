# Remote Mic — Windows client (RC003 build candidate)

> **状态：源码/构建候选，尚需真实 RC003 遥控器验收。** 本目录包含跨平台
> 协议测试，以及针对 WinRT BLE、Raw Input、SendInput 和 PortAudio 的
...
> 适用范围和限制与上方状态说明一致：本候选**尚未完成 Windows 真机验收**。
...
- 设置窗口使用 PySide6 Essentials + Qt Quick/QML；便携版和安装器都通过 PyInstaller 打包，不要求终端用户另装 Python 或 Qt。同一个 `RemoteMicRC003.exe` 用 `--settings` 打开设置窗口、不带参数启动桥接。
...
- VB-CABLE 的安装会改变系统状态并可能需要重启，属于用户可选的第三方软件流程，
  不是 Remote Mic 的必需依赖。也可以完全跳过它，使用其他明确选定的音频端点。
...
Windows 候选版应以预发行版发布。每个版本的安装器、便携版 ZIP 和
`SHA256SUMS.txt` 必须来自同一次 Windows CI 构建；发布前仍应在真实 RC003 上完成
