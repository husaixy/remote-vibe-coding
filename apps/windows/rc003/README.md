...
"保存并重启桥接"（见下方"首次使用"一节）。安装器的 Start Menu
...
1. 打开"设置"（Start Menu 中的"Remote Vibe Coding"或
   ...
   上一节配置好的端点；
2. 启动桥接有两种方式：在设置窗口底部"桥接控制"区域点击"保存并
   重启桥接"（会先校验并保存设置；已有桥接时先完成断开和清理，再启动
   新桥接；如果蓝牙重连后 Windows 将遥控器 HID 实例留在禁用状态，会重新
   启用唯一匹配的 RC001/RC003 并等待按键通道恢复，必要时显示一次 UAC），
   或者关闭设置窗口后从 Start Menu 选择"启动
...
2. 启动桥接有两种方式：在设置窗口底部"桥接控制"区域点击"保存并
   重启桥接"（先保存，已有桥接时先断开再启动）；或者关闭设置窗口，在同一个
...
**需要手动完成一次 Codex 快捷键绑定。仅安装或重启 Remote Vibe Coding 不会自动完成此项配置。**

1. 打开 Codex 的 **设置 → 键盘快捷键（Keyboard Shortcuts）**。
2. 搜索 **聚焦主聊天 / Focus main chat**，录入 **`Ctrl+Alt+Shift+F12`** 并保存。如提示冲突，先处理冲突，再确保该命令使用这一组合键。桥接目前固定发送此组合键，只修改 Codex 一端为其他按键无法联动。
3. 打开已有可输入的对话，把焦点移出输入框，再用普通键盘按该组合键。输入几个字验证文字进入主对话输入框，无需发送。如果键盘测试不通过，先解决 Codex 侧绑定。
4. 打开 Remote Vibe Coding 的“按键映射”，将主页键单击设为“收起 Codex”，长按设为“唤醒 Codex 并聚焦输入框”，保存并重启桥接。新安装和恢复默认会采用这组动作；升级不会覆盖已有自定义映射。无需为此恢复全部默认。
5. 关闭 Remote Vibe Coding 设置窗口，在 Codex 前台短按主页键，应最小化 Codex；在其他应用前台长按主页键约 0.55 秒，应恢复或启动 Codex，并发送聚焦快捷键。
6. 确认光标进入输入框后，按住语音键说话、松开结束。输入法的语音快捷键与音频设备仍需按前文配置，主页键绑定不会自动配置语音输入。

| 现象 | 检查方法 |
| --- | --- |
| 短按主页键没有反应 | 只有 Codex 位于前台才会收起；设置窗口或其他软件在前台时无动作是正常行为。 |
| 长按能打开 Codex，但输入框没有焦点 | 先用键盘验证 `Ctrl+Alt+Shift+F12`，确认命令绑定和当前对话可输入。日志显示快捷键已发送不等于输入框已成功聚焦。 |
| 升级后仍执行旧主页键动作 | 检查现有单击/长按映射并保存，旧配置不会被强制迁移。 |
| 语音可用，但主页键和其他普通按键都失效 | 点击“保存并重启桥接”，等待 HID 枚举；必要时会出现设备修复的 UAC 提示。按普通键后再检查日志。 |
| 输入框已有焦点，但语音没有文字 | 先确认输入法语音能单独工作，再核对本程序输出 `CABLE Input`、输入法输入 `CABLE Output`。 |

“桥接运行中”仅代表进程存活；`RC003 HID report tap enabled` 仅代表监听已启动。
日志出现 `first RC003 HID report received; buttons healthy` 才证明已收到真实按键报告。
若恢复后仍无反应，在“打开日志目录”查看 `app.log`，记录测试时间和动作以便定位。

快捷键设置入口参考 [OpenAI 官方快捷键说明](https://learn.chatgpt.com/docs/reference/commands)。
“Focus main chat”名称来自本项目已验证的 Codex 版本；不同版本或语言的名称可能变化，
