...
新安装或点击“恢复默认”后，主页键短按会在 Codex 位于前台时将窗口最小化；
长按约 0.55 秒会恢复或启动 Codex，并聚焦当前主对话输入框。首次使用前，请在
Codex 的可见快捷键设置中把“聚焦主聊天 / Focus main chat”绑定为
`Ctrl+Alt+Shift+F12`。桥接只发送该公开快捷键，不读取或修改 Codex 私有配置。
