...
class HostHotkeyFailureSuppressesMicOpenTests(_AppWiringTestCase):
    ...
    def test_right_alt_hold_audio_start_uses_hidden_toolbar(self):
    ...
    def test_wechat_input_method_right_alt_hold_uses_hidden_toolbar(self):
        self.app._voice.trigger_mode = key_mapping.VoiceTriggerMode.HOLD
        self.app._voice_hotkey = app_module.hotkey.HotkeySpec.parse("ralt")
        panel_calls = []
        keyboard_calls = []
    ...
    def test_wechat_input_method_right_alt_closes_through_same_toolbar(self):
        self.app._voice.trigger_mode = key_mapping.VoiceTriggerMode.HOLD
        self.app._voice_hotkey = app_module.hotkey.HotkeySpec.parse("ralt")
        panel_calls = []
        keyboard_calls = []
    ...
    def test_failed_hold_activation_is_not_retried_by_duplicate_channels(self):
        self.app._voice.trigger_mode = key_mapping.VoiceTriggerMode.HOLD
        self.app._voice_hotkey = app_module.hotkey.HotkeySpec.parse("ralt")
        panel_calls = []
        original_panel = app_module.wechat_input_method.set_voice_panel_active
        app_module.wechat_input_method.set_voice_panel_active = (
            lambda active: panel_calls.append(active) or False
        )
        try:
            self.app._on_control_event(AudioStarted(session_id=1))
            self.app._on_legacy_key_event(0x74, True)
...
class OrdinaryButtonGestureWiringTests(_AppWiringTestCase):
    ...
    def test_codex_window_actions_use_their_dedicated_executors(self):
        calls = []
        original_minimize = app_module.minimize_codex_if_foreground
        original_focus = app_module.focus_codex_main_chat
        app_module.minimize_codex_if_foreground = lambda: calls.append("minimize") or True
        app_module.focus_codex_main_chat = lambda: calls.append("focus") or True
        try:
            self.app._apply_button_action(
                key_mapping.ButtonAction(key_mapping.ActionKind.MINIMIZE_CODEX)
            )
            self.app._apply_button_action(
                key_mapping.ButtonAction(key_mapping.ActionKind.FOCUS_CODEX_MAIN_CHAT)
            )
        finally:
            app_module.minimize_codex_if_foreground = original_minimize
