...
- 根 README 重写项目来源、二次开发内容、实现效果和完整使用流程；直接列出 Codex Micro 预设所需的全部 Codex 快捷键，并补充 VB-CABLE、输入法、HID 权限、桥接重启和音量键占用等注意事项。
- 增加极简扁平化设置窗口交互稿和实现边界说明，供下一轮 UI 评审。
- 根 README 增加 Codex 首次快捷键绑定入口；客户端 README 补充手动绑定、主页键映射、键盘验证、语音联动及按键通道排障步骤。

### 新增

- 按键映射页新增“Codex Micro 风格预设”：上下键切换最近会话、音量键调节推理强度、确认键发送/长按批准、返回键长按拒绝，并提供侧边栏、搜索会话、Fork、新建会话、Fast 和 Plan 模式动作。除确认键在聚焦输入框后使用原生 `Enter` 外，其他 Codex 命令通过用户手动绑定的公开快捷键执行。
...
- Windows 可执行文件、安装程序、开始菜单和桌面快捷方式现在统一使用遥控器与语音波纹图标，并内置 16–256 px 多尺寸资源，改善任务栏和桌面小图标的清晰度。
- Windows 设置窗口采用新的极简扁平视觉：增加遥控器与语音组合应用图标，统一轻量字重、浅色语义色板、细分隔线和顶部导航；按键映射页保留可点击实物图，并改为一次集中编辑一个按键的单击、双击和长按动作。
- 连接、权限和检查页面使用相同的页面标题、留白、圆角与状态色规范；窗口仍使用 Windows 原生标题栏，保留拖动、缩放和高 DPI 行为。

### 修复

- 修复新版 Codex 主输入框中语音转写完成后，遥控器确认键因依赖未配置的 `Ctrl+Alt+Shift+Enter` 而无法发送的问题；现在确认键会先聚焦主聊天，等待界面稳定后直接发送原生 `Enter`，无需额外绑定 `Send message`。
- 修复桥接运行时按住电脑键盘方向键会在全局键盘钩子中逐个累积遥控器识别等待，导致视频网站松开右方向键后仍持续倍速数秒的问题；现在只在一次按键按下的首个事件上等待设备关联，后续自动重复和松开事件立即沿用同一归属。
