...
- 主页键新增 Codex 专用默认手势：短按仅在 Codex 位于前台时将其最小化，长按约 0.55 秒恢复或启动 Codex，并在确认窗口位于前台后发送 `Ctrl+Alt+Shift+F12` 聚焦主聊天输入框。Codex 端需通过可见的快捷键设置将“聚焦主聊天 / Focus main chat”绑定为该组合键。

### 修复

- “保存并重启桥接”现在会在 BLE 已恢复但 Windows HID 设备仍为问题码 22 时重新启用唯一匹配的 RC001/RC003，并等待 HID 路径出现后再启动按键监听；普通权限不足时仅由这次明确点击触发短时 UAC 修复助手。自动重连也会等待延迟枚举，避免长期停在“语音可用、普通按键失效”的半连接状态。
- “打开 Codex”不再误启动 Windows 应用包内的命令行后端 `codex.exe`，而是解析真正的桌面 GUI `ChatGPT.exe`。
